...
% 
% jackal_n3_01_H10_noSlack_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the jackal_n3_01_H10_noSlack FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% jackal_n3_01_H10_noSlack_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% jackal_n3_01_H10_noSlack_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% jackal_n3_01_H10_noSlack_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function jackal_n3_01_H10_noSlack_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'jackal_n3_01_H10_noSlack_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'jackal_n3_01_H10_noSlack_block';
    end
    ...
    function_name = 'jackal_n3_01_H10_noSlack_coderFunction.m';
    position = [170, 99, 650, 350];
    parameter_sizes = struct('x0', '[100,1]',...
                             'xinit', '[8,1]',...
                             'all_parameters', '[270,1]');
    if useCompactBlock
        function_name = 'jackal_n3_01_H10_noSlack_coderFunctioncompact.m';
